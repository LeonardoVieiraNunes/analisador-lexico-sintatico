...
def LL():
    ...
    while input_string != "-1":
        input_string = input("Digite o input utilizando espaco entre os simbolos\n")
        if input_string != "-1":
            validity = validateStringUsingStackBuffer(parsing_table, result, tabTerm, input_string, term_userdef, start_symbol)
            print(validity)
...
def validateStringUsingStackBuffer(parsing_table, grammarll1,
    ...
    while True:
        ...
        if stack == ['$'] and buffer == ['$']:
            ...
            if stack[0] not in diction.keys() or buffer[-1] not in table_term_list:
...
if __name__ == "__main__":
    ...
    operation = 0
    while operation != "-1":
        operation = input("\nDigite o numero da opção referente a operação que deseja realizar(digite -1 para parar)\n"
                          "1. Eliminar recurcao a esquerda\n"
                          "2. Fatorar\n"
                          "3. Reconhecer sentenca usando LL\n"
                          "4. Reconhecer sentenca usando SLR\n")
