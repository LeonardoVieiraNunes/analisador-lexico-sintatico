...
class MyDeterminationOfInput(MyOperatingJSON):
    ...
    def union(self, final_automata: MyOperatingJSON, afd1: MyOperatingJSON, afd2: MyOperatingJSON):
        self.new_automata = final_automata
    ...
    def create_epsilon_transitions(self):
        transitions = self.new_automata.get_transitions()
        for state in self.new_automata.get_states():
    ...
    def create_new_automata(self, operation: str):
        new_states_set_list = [self.epsilon_transitions.__getitem__(self.new_automata.get_initial())]
            ...
            if operation == 'intersection':
                for state in current_state_set:
                    if (state in self.temp_automata_1.get_final()) and (state in self.temp_automata_2.get_final()):
                        temporary_data['final_state'].add(''.join(sorted(current_state_set)))
                        break
            for symbol in self.new_automata.get_symbols():
                temp_to_state_set = set()
                for state in current_state_set:
                    for transition in self.new_automata.get_transitions():
        ...
        self.new_automata.data = temporary_data
